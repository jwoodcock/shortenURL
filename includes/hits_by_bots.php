<?php 
	...
	if ($pageA[3]){
		// GET HIT DETAILS
		$u = "SELECT userAgent, returner, referrer, shorturl, ipAddress, DATE_FORMAT(dateHit,'%m/%d/%Y %H:%i') as uDate, hId, countryCode, country, city, region, isp FROM url_hits WHERE userAgent like '%{$pageA[3]}%' ORDER BY dateHit DESC";
		$uSQL = mysql_query($u);
